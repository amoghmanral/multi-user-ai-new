...
export const ChatInputSchema = z.object({
  ...
  messageType: z.enum(['text', 'voice', 'ai']).optional(),
});
...
const callAgent = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    // Build room context if roomId is provided
    ...
    if (roomContext) {
      runtimeContext.set('roomContext', roomContext);
    }

    const messages = [
      'User message: ' + prompt,
      'Additional context (for background knowledge): ' + JSON.stringify(additionalContext),
      roomContext ? 'Room context: ' + JSON.stringify(roomContext) : '',
    ].filter(Boolean);
    ...
    const streamResult = await starterAgent.streamVNext(messages, {
      ...
      ...(enhancedSystemPrompt ? ({ instructions: enhancedSystemPrompt } as const) : {}),
      modelSettings: {
        temperature,
